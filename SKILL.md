...
description: Create reusable PowerPoint template style profiles and generate native editable PPTX decks from those profiles, outlines, and content. Use when the user provides a PPT/PPTX template, names an existing style profile, asks to generate slides that follow a template, or wants LaTeX formulas preserved across HTML preview and PPTX export. Always run dependency checks first, never auto-install software, extract or reuse a template style profile, clean XML evidence before AI summarization, choose direct template mode by default, use frontend-slides only for optional creative previews, author dom-to-pptx-compatible HTML, export through dom-to-pptx, and validate the generated PPTX.
---
...
- LaTeX is the canonical source for mathematical formulas; HTML and PPTX must render from the same math manifest.
...
Dependency detection is read-only. If runtime dependencies are missing, report the missing items and point the user to `INSTALL.md`. Do not install system software, npm packages, browsers, or companion Skills automatically.
...
Flow: preflight -> extract or reuse profile -> normalize inputs -> slide plan -> math manifest -> dom-to-pptx HTML -> PPTX export -> validation -> delivery.
...
Flow: preflight -> extract or reuse profile -> normalize inputs -> slide plan -> math manifest -> three first-slide HTML previews -> user selects or mixes directions -> full dom-to-pptx HTML -> PPTX export -> validation -> delivery.
...
1. **Preflight dependencies**
   ...
   - Required: Python 3.10+, Node.js 20+, npm or pnpm, `dom-to-pptx`, `pptxgenjs`, and Playwright Chromium or equivalent.
   - Companion Skills: `frontend-slides` and `dom-to-pptx` should be installed manually for best results.
   - See `INSTALL.md` and `references/dependency-routing.md`.
...
2. **Extract or reuse template style profile**
   ...
   - Extract raw XML evidence first, clean and normalize it deterministically, then use AI to summarize style rules and cleanup decisions.
   ...
   - See `references/template-style-profile.md`, `references/ai-style-summarization.md`, and `references/style-profile-reuse.md`.
...
3. **Normalize input**
   ...
   - Detect inline and block LaTeX before visual HTML authoring.
   - See `references/input-contract.md` and `references/math-formula-pipeline.md`.
...
4. **Create slide plan**
   ...
   - Formula content blocks must reference math manifest IDs rather than duplicating renderer-specific output.
   - See `references/workflow.md` and `references/slide-plan-schema.md`.
...
6. **Author dom-to-pptx-compatible HTML**
   ...
   - Represent formulas as stable LaTeX-backed math nodes in HTML.
   - Avoid editor UI, hidden controls, debug labels, hover states, animations, and unsupported CSS effects in the export DOM.
   - See `references/dom-to-pptx-contract.md` and `references/math-formula-pipeline.md`.
...
7. **Export PPTX**
   ...
   - For formulas, default to SVG math assets with original LaTeX preserved in alt text or metadata.

8. **Validate and repair**
   - Run `scripts/validate_pptx.py` on every generated PPTX.
   - Check zip integrity, XML parseability, slide count, relationships, content types, media references, formula asset references, and notes cleanup.
...
- Never auto-install system software, npm packages, browsers, or companion Skills.
...
- Never let AI overwrite raw XML extraction facts; AI may summarize and add an overlay only after deterministic cleaning.
...
- `references/dependency-routing.md`: dependency detection and fallback rules.
- `references/template-style-profile.md`: profile extraction schema and principles.
- `references/ai-style-summarization.md`: AI summarization and cleanup contract after XML parsing.
...
- `references/math-formula-pipeline.md`: LaTeX formula manifest and rendering pipeline.
